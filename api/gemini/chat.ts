...
**Selection Process:**
1. **Phase 1: PRD CREATION (MANDATORY).** For ANY new project request, respond with \`"responseType": "PROJECT_PLAN"\` that includes a complete PRD with:
    - Selected template name and why it matches user's request
    - Detailed backend requirements: API endpoints, database schema, authentication, services, file structure
    - Detailed frontend requirements: Components, state management, routing, styling, file structure
    - Detailed standalone HTML requirements: HTML structure, CSS integration, JS functionality, features, quality standards
    - Tech stack emphasizing React complexity for frontend and appropriate backend technologies
2. **Phase 2: MULTI-AGENT CODE GENERATION.** Only after PRD approval, generate three simultaneous \`"responseType": "MODIFY_CODE"\` responses:
    - Frontend Agent: Complete React application with multiple files and components
    - Backend Agent: API endpoints, services, database setup, and backend logic
    - Standalone Agent: Full conversion to standalone HTML that replicates ALL React functionality using vanilla JavaScript with high quality
...
*   **Part B: Complete Standalone HTML Conversion (\`standaloneHtml\` string):** You MUST generate a high-quality standalone HTML file that is a COMPLETE conversion of the React app into vanilla JavaScript. This must:
    - Replicate EVERY feature, interaction, and UI element from the React app with pixel-perfect accuracy
    - Use sophisticated vanilla JavaScript with advanced state management, component system, and reactivity
    - Include all CSS styling inline or embedded, ensuring responsive design and modern aesthetics
    - Be a fully functional "mini React" built with pure JavaScript, including form validation, real-time updates, and complex interactions
    - Have identical functionality to the original React app, with butter-smooth animations and professional polish
    - Meet the highest quality standards: no placeholder content, all buttons functional, persistent data with localStorage, and comprehensive error handling
...
const RESPONSE_SCHEMA = {
    ...
    properties: {
        ...
        plan: {
            type: Type.OBJECT,
            description: "The complete Product Requirements Document (PRD). Only used when responseType is 'PROJECT_PLAN'.",
            properties: {
                ...
                backendRequirements: {
                    type: Type.OBJECT,
                    description: "Detailed specifications for backend code generation.",
                    properties: {
                    ...
                    required: ['apiEndpoints', 'databaseSchema', 'authentication', 'services', 'fileStructure']
                },
                frontendRequirements: {
                    type: Type.OBJECT,
                    description: "Detailed specifications for frontend code generation.",
                    properties: {
                    ...
                    required: ['components', 'stateManagement', 'routing', 'styling', 'fileStructure']
                },
                standaloneRequirements: {
                    type: Type.OBJECT,
                    description: "Detailed specifications for standalone HTML conversion.",
                    properties: {
                        htmlStructure: { type: Type.STRING },
                        cssIntegration: { type: Type.STRING },
                        jsFunctionality: { type: Type.STRING },
                        features: { type: Type.ARRAY, items: { type: Type.STRING } },
                        qualityStandards: { type: Type.ARRAY, items: { type: Type.STRING } }
                    },
                    required: ['htmlStructure', 'cssIntegration', 'jsFunctionality', 'features', 'qualityStandards']
                }
            },
            required: ['projectName', 'description', 'features', 'techStack', 'backendRequirements', 'frontendRequirements', 'standaloneRequirements']
        },
        modification: {
            type: Type.OBJECT,
